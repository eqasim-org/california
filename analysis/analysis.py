...
def import_data_actual(context, counties):
    # Actual data
    df_act_persons = context.stage("data.hts.cleaned")[0]
    df_act_trips = context.stage("data.hts.cleaned")[1]
    ...
    df_act = df_act_trips.merge(df_act_persons[["person_id", "weight_person", "employed", 
                                                "sex", "number_of_cars", "sf_home", "home_county_id"]],
    ...
    df_persons_no_trip = df_persons_no_trip.set_index(["person_id"])
    #df_act = df_act[df_act["home_county_id"].isin(counties)]
    #df_persons_no_trip = df_persons_no_trip[df_persons_no_trip["home_county_id"].isin(counties)]
...
def aux_data_frame(df_act):
    ...
    df2 = df_act[["origin_purpose_first", "destination_purpose_first", "weight_person"]].groupby(level="person_id")[
    ...
    df_aux = pd.merge(df_act["weight_person"].reset_index(), df2, on='person_id').drop_duplicates("person_id")
    return df_aux
...
def mode_purpose_comparison(context, df_syn, df_act, suffix = None):
    ...
    types = df_syn.groupby(["mode","destination_purpose"]).count()["person_id"]
    syn = types / types.sum()
    ...
    #SD
    #which = ["car", "car_passenger", "pt", "walk"]
...
def compute_distances_synthetic(df_syn, threshold = 25):
    ...
    df_syn["crowfly_distance"] = df_syn["crowfly_distance"] / 3280
...
def compute_distances_actual(df_act, threshold = 25):   
    ...
    df_act_dist = df_act[df_act["crowfly_distance"] / 3280 < threshold]
    df_act_dist["crowfly_distance"] = df_act_dist["crowfly_distance"] / 3280
    return df_act_dist


def compare_dist_educ(context, df_syn, df_act, suffix = None):
    pers_educ_syn = list(set(df_syn[df_syn["destination_purpose"] == "education"]["person_id"].values))
    ...
    df_syn_h_e = df_syn_educ[np.logical_or( np.logical_and( df_syn_educ["origin_purpose"] == "home",  df_syn_educ["destination_purpose"] == "education" ), np.logical_and(df_syn_educ["destination_purpose"] == "home",  df_syn_educ["origin_purpose"] == "education")     )]
...
def execute(context):
    ...
    df_act, df_act_no_trip = import_data_actual(context, counties)
    ...
    syn = df_syn_dist.groupby(["destination_purpose"]).mean()["crowfly_distance"] 
    ...
    synM = df_syn_distM.groupby(["destination_purpose"]).mean()["crowfly_distance"] 
    ...
    synW = df_syn_distW.groupby(["destination_purpose"]).mean()["crowfly_distance"] 
