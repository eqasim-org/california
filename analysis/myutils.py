...
def process_synthetic_activity_chain_counts(df_syn):
    ...
    # Split destination_purpose into subarrays grouped by person_id
    larr = np.split(mdf_syn["destination_purpose"].values, np.cumsum(mdf_syn["destination_purpose"].groupby(level='person_id').size()))

    # Split origin_purpose into subarrays grouped by person_id
    arr = np.split(mdf_syn["origin_purpose"].values, np.cumsum(mdf_syn["origin_purpose"].groupby(level='person_id').size()))
...
def impute_geo(df, df_zones, origin):
    ...
    df_geo = gpd.GeoDataFrame(df, crs = {"init" : "epsg:2227"})
